...
def from_target(target: np.ndarray, initial_t: float = 0, initial_y: float = None, initial_z: float = None, initial_u: float = None) -> np.ndarray:
    """
    Returns the initial position and velocity for an orbit that reaches the target position at the target time, leaving
      at initial time
    ...
    def trajectory_constraint(u0_inner, d0_inner, mu, x0, d1_inner, x1) -> float:
        return ((d0_inner * u0_inner) ** 2)/mu - np.hypot(x1, d1_inner) -\
            ((x0 * x1 - d1_inner * d0_inner) / np.hypot(x0, d0_inner) + d0_inner * d1_inner * u0_inner * u0_inner / mu)
    ...
    def time_constraint(u0_inner, d0_inner, mu, x0, r1, side_of_periapse, dt) -> float:
        ...
        return np.sqrt(np.abs(sma) ** 3 / mu)*((ecc * np.sinh(h1) - h1) - (ecc * np.sinh(h0) - h0)) - dt

    def traj_derivatives(u0_inner, d0_inner, mu, x0, d1_inner, x1) -> tuple[float, float]:
        dtraj_du = 2*u0_inner*d0_inner*(d0_inner - d1_inner)/mu

        r0 = np.hypot(d0_inner, x0)
        dtraj_dd = (2*u0_inner*u0_inner*d0_inner/mu) + (x0*x1*d0_inner/r0**3) -\
                   (d1_inner*(1-(d0_inner/r0)**2)/r0) + u0_inner*u0_inner*d1_inner/mu
        return dtraj_du, dtraj_dd

    def time_derivatives(u0_inner, d0_inner, mu, x0, r1, side_of_periapsis, _) -> tuple[float, float]:
        de_du = u0_inner
        ...
        dh0_du = -(da_du*(ecc*np.cosh(h0)-1) + sma*np.cosh(h0)*decc_du)/(sma*ecc*np.sinh(h0))
        dh0_dd = -(da_dd*(ecc*np.cosh(h0)-1) + sma*np.cosh(h0)*decc_dd)/(sma*ecc*np.sinh(h0))
        ...
        if h1 == 0:
            ...
            dh1_du = -(da_du * (ecc * np.cosh(h1) - 1) + sma * np.cosh(h1) * decc_du) / (sma * ecc * np.sinh(h1))
            dh1_dd = -(da_dd * (ecc * np.cosh(h1) - 1) + sma * np.cosh(h1) * decc_dd) / (sma * ecc * np.sinh(h1))
    ...
    # initial guess is close enough in to pass within the target orbit, with enough speed to definitely be hyperbolic
    vel = np.sqrt(40 + np.abs(2*Sun.gravity/x0_km)) if initial_u is None else initial_u
    dist = 0.9*d1 if initial_y is None or initial_z is None else np.hypot(initial_y, initial_z)
    guess = np.array([vel, dist])
    before_periapse = target[2] < 0 # if y-component is < 0 it's on the rising side of periapsis
    ...
    dx = newton_solve(d1, error, guess, target,
                      time_derivatives, traj_derivatives, x0_km, initial_t)
...
def newton_solve(d1, error, initial_guess, target, time_derivatives, traj_derivatives,
                 x0_km, t0) -> np.ndarray:
